...
void user_tws_debug(){
    ...
    if (tws_api_get_role() == TWS_ROLE_MASTER) {
        y_printf("TWS_ROLE_MASTER\n");
    }else{
        y_printf("TWS_ROLE_SLAVE\n");
    }
    y_printf("**********************************\n");

    extern u8 tws_auto_role_switch;
    y_printf("tws_auto_role_switch %d\n",tws_auto_role_switch);

    y_printf("tws_api_get_local_addr\n");
    tws_api_get_local_addr(mac_addr);
    put_buf(mac_addr,6);
    y_printf("tws_api_get_sibling_addr\n");
    tws_api_get_sibling_addr(mac_addr);
    put_buf(mac_addr,6);


    y_printf("local_channel %d-%d %d %d\n\n",tws_api_get_local_channel(),'L','R','U');
}

void bt_tws_delete_pair_timer()
{
    if (gtws.pair_timer) {
...
void bt_tws_sync_volume()
{
    ...
    tws_api_send_data_to_sibling(data, 1, TWS_FUNC_ID_VOL_SYNC);
}
...
static void tws_vol_up_init(void)
{
    bt_mute_flag = 0;
}
...
static void tws_poweroff_same_time_deal(void)
{
    ...
    int ret = tone_play_with_callback_by_name(tone_table[IDEX_TONE_POWER_OFF], 1,
              /*tws_poweroff_same_time_tone_end*/power_mute_init, (void *)IDEX_TONE_POWER_OFF);
    if (ret) {
        sys_enter_soft_poweroff(NULL);
    }
}
...
static void tws_event_sync_fun_cmd(struct bt_event *evt)
{
    ...
    case SYNC_CMD_TWS_CLEAK_TWS:
         bt_disconnect_tws_conn();
        break;

    case SYNC_CMD_MAX_UP:                 //TWS下，抬起解MUTE
        tws_vol_up_init();
        break;
...
{
    u8 addr[6], addr_temp[6];

    if (channel == 'L') {
        memcpy(common_addr, bt_get_mac_addr(), 6);
    } else {
        addr[0] = remote_addr[5];
        addr[1] = remote_addr[4];
        addr[2] = remote_addr[3];
        addr[3] = remote_addr[2];
        addr[4] = remote_addr[1];
        addr[5] = remote_addr[0];
        memcpy(common_addr, addr, 6);
    }
}
#endif
...
//注册TWS同步信息
REGISTER_TWS_FUNC_STUB(app_tws_sync_stub) = //注册 app_tws_sync_stub 类
{
    .func_id = USER_TWS_FUNC_ID_USER_INFO_SYNC,//自定义ID号
    .func    = bt_tws_user_info_sync,//回调函数
};
#endif //USER_TWS_DATA_SEND_TEST
