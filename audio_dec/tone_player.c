...
//     return TONE_STOP;
// }
u8 tone_play_status = 0;
extern u8 mic_delay;
extern u8 bool_change;
u8 mic_play_online = 0;
/**
 * @brief 获取当前音频状态
 * 
 * @return int 返回音频状态，TONE_START表示音频开始，TONE_STOP表示音频停止
 */
int tone_get_status(void)
{
    // 如果tone_dec存在且其cur_list不为空
    if (tone_dec && tone_dec->cur_list) {
#if AUDIO_VOCAL_REMOVE_EN
        // 如果音频播放状态为0（未播放）
        if(!tone_play_status)
        {
            // 如果麦克风效果正在运行，则停止麦克风效果
           if (mic_effect_get_status()) {
               mic_effect_stop();
           }

           // 如果bool_change为真，调用musc_remove_power_stop函数
           if(bool_change)
           {
              void musc_remove_power_stop(void);
    ...
    if(tone_play_status)
    {
           // 如果bool_change为真，调用musc_remove_power_start函数
           if(bool_change)
           {
              void musc_remove_power_start(void);
              musc_remove_power_start();
           }
