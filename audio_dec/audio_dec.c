...
static void audio_phase_inver_output_data_process_len(struct audio_stream_entry *entry,  int len)   //音频反向数据处理
{
}
...
int audio_dac_L_energy_get(void)
{
#if AUDIO_OUTPUT_AUTOMUTE
    int audio_energy_detect_energy_get(void *_hdl, u8 ch);
    if (mix_out_automute_hdl) {
...
#else
    return 0;
#endif // AUDIO_OUTPUT_AUTOMUTE
}

int audio_dac_R_energy_get(void)
{
#if AUDIO_OUTPUT_AUTOMUTE
    int audio_energy_detect_energy_get(void *_hdl, u8 ch);
    if (mix_out_automute_hdl) {
           return audio_energy_detect_energy_get(mix_out_automute_hdl, BIT(1));
    }
...
int ave_dac_energy_get(void)
{
    int r_eny = audio_dac_R_energy_get();
    int l_eny = audio_dac_L_energy_get();
    //printf("--r_eny = %d  l_eny=%d\n ",r_eny,l_eny);
    return (r_eny>l_eny?r_eny:l_eny);
}
//////////////////////////////////////////////////////////////////////////////
/*----------------------------------------------------------------------------*/
/**@brief    激活所有解码
...
void musc_remove_power_start(void)            //人声消除开始
{
    audio_vocal_remove_sw(mix_vocal_remove_hdl,0);
}

void musc_remove_power_stop(void)            //人声消除关闭
{
    audio_vocal_remove_sw(mix_vocal_remove_hdl,1);
}

u8 bool_change = 0;
void musc_remove_colse(void)               //按键控制消音开启
{
        ...
        if(bool_change)
        {
           musc_remove_power_stop();
           tone_play_with_callback_by_name(tone_table[IDEX_TONE_ECHO_START], 1, 0, (void *)IDEX_TONE_ECHO_START);
        }else
        {
           musc_remove_power_stop();
           tone_play_with_callback_by_name(tone_table[IDEX_TONE_ECHO_STOP], 1, 0, (void *)IDEX_TONE_ECHO_STOP);
        }
}
#endif
