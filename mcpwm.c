...
{
    timer_pwm_init(JL_TIMER2, 10000, 0, IO_PORTA_00, CH2_T2_PWM_OUT);
    timer_pwm_init(JL_TIMER3, 10000, 0, IO_PORTB_05, 0);
    timer_pwm_init(JL_TIMER5, 10000, 0, IO_PORTB_07, 0);
}

//如果开启PWM后，产生杂音，可以把频率设置在20K以上
//开启PWM，每个定时器对应相应的映射通道，比如：定时器2对应 CHX_T2_PWM_OUT
void timer_pwm2_set(u16 PWM ,u8 IO_XX)
{
    timer_pwm_init(JL_TIMER2, 10000, PWM, IO_XX, CH2_T2_PWM_OUT);
}

void timer_pwm3_set(u16 PWM ,u8 IO_XX)
{
    timer_pwm_init(JL_TIMER5, 10000, PWM, IO_XX, CH0_T5_PWM_OUT);
}
