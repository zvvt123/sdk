...
int esco_enc_open(u32 coding_type, u8 frame_len)
{
    ...
    if (get_t_fixed_dac_output_rate16k()) {
...
#if TCFG_MIC_EFFECT_ENABLE
#define MIC_ADC_IRQ_POINTS    256 //((MIC_EFFECT_SAMPLERATE/1000)*4)  //不能随便更改
#else
...
{
    u16 irq_point_unit = MIC_ADC_IRQ_POINTS;
    ...
    if (mic_var.init_flag) {
        ...
        mic_var.adc_buf = (s16 *)zalloc(irq_point_unit * 2 * MIC_ADC_BUF_NUM);
        mic_var.states = 0;
    }
    ...
    if (mic_var.states == 0) {
        ...
        audio_adc_mic_set_buffs(mic, mic_var.adc_buf,
                                irq_point_unit * 2, MIC_ADC_BUF_NUM);
