...
struct hw_eq {
    struct list_head head;            //链表头(该链表中途会有add del的操作，不用于遍历查找激活需要激活的eq)
    struct list_head head2;           //链表头(遍历查找激活需要激活的eq)
    OS_MUTEX mutex;                   //互斥锁
...
struct hw_eq_ch {
    unsigned char out_32bit;          //是否输出32bit位宽的数据  0：16bit 1:32bit
    ...
    unsigned char channels;           //通道数（1或2）
    ...
    struct list_head entry2;          //当前eq通道的节点
...
*/
/*----------------------------------------------------------------------------*/
int eq_seg_design(struct eq_seg_info *seg_tmp, int sample_rate, int *coeff);
