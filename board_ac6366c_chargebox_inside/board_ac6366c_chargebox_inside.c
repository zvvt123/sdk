...
#ifdef CONFIG_BOARD_AC6366C_CHARGEBOX_INSIDE
...
LED_PLATFORM_DATA_BEGIN(pwm_led_data)
	...
	.io_cfg.two_io.pin0 = TCFG_PWMLED_PIN0,     //两个IO推两个灯的IO口配置
	.io_cfg.two_io.pin1 = TCFG_PWMLED_PIN1,     //两个IO推两个灯的IO口配置
...
LED_PLATFORM_DATA_BEGIN(chgbox_pwm_led_data)
	.io_mode = LED_TWO_IO_MODE,               //推灯模式设置
	.io_cfg.two_io.pin0 = NO_CONFIG_PORT,     //两个IO推两个灯的IO口配置
	.io_cfg.two_io.pin1 = NO_CONFIG_PORT,     //两个IO推两个灯的IO口配置
LED_PLATFORM_DATA_END()
#endif
...
struct port_wakeup key_port = {
    ...
    .filter_enable = 1,                                    //滤波使能
};

#if TCFG_CHARGE_BOX_ENABLE && TCFG_CHARGE_MOUDLE_OUTSIDE
struct port_wakeup usbin_det_port = {
    .pullup_down_enable = DISABLE,                         //配置I/O 内部上下拉是否使能
    .edge       = RISING_EDGE,                             //唤醒方式选择,可选：上升沿\下降沿
    .attribute  = BLUETOOTH_RESUME,                        //保留参数
    .iomap      = TCFG_USB_ONLE_DET_IO,                    //唤醒口选择
    .filter_enable = 1,                                    //滤波使能
};
struct port_wakeup charge_full_det_port = {
    .pullup_down_enable = DISABLE,                         //配置I/O 内部上下拉是否使能
    .edge       = RISING_EDGE,                             //唤醒方式选择,可选：上升沿\下降沿
    .attribute  = BLUETOOTH_RESUME,                        //保留参数
    .iomap      = TCFG_CHARGE_FULL_DET_IO,                 //唤醒口选择
    .filter_enable = 1,                                    //滤波使能
};
#endif
...
struct port_wakeup hall_det_port = {
    ...
    .edge       = RISING_EDGE,                             //唤醒方式选择,可选：上升沿\下降沿
    .attribute  = BLUETOOTH_RESUME,                        //保留参数
    .iomap      = TCFG_HALL_PORT,                          //唤醒口选择
    .filter_enable = 1,                                    //滤波使能
};

#if (TCFG_CHARGE_BOX_ENABLE && TCFG_WIRELESS_ENABLE)
struct port_wakeup wl_det_port = {
    .pullup_down_enable = DISABLE,                         //配置I/O 内部上下拉是否使能
    .edge       = RISING_EDGE,                             //唤醒方式选择,可选：上升沿\下降沿
    .attribute  = BLUETOOTH_RESUME,                        //保留参数
    .iomap      = TCFG_WL_AD_DET_IO,                       //唤醒口选择
    .filter_enable = 1,                                    //滤波使能
};
#endif

#if (TCFG_TEST_BOX_ENABLE && (TCFG_CHARGESTORE_PORT != IO_PORTB_05))
struct port_wakeup uart_update_port = {
    .pullup_down_enable = DISABLE,                         //配置I/O 内部上下拉是否使能
    .edge       = FALLING_EDGE,                            //唤醒方式选择,可选：上升沿\下降沿
    .attribute  = BLUETOOTH_RESUME,                        //保留参数
...
#if (TCFG_CHARGE_BOX_ENABLE && TCFG_SHORT_PROTECT_ENABLE)
struct port_wakeup short_det_port = {
    .pullup_down_enable = DISABLE,                         //配置I/O 内部上下拉是否使能
    .edge       = FALLING_EDGE,                            //唤醒方式选择,可选：上升沿\下降沿
    .attribute  = BLUETOOTH_RESUME,                        //保留参数
    .iomap      = TCFG_SHORT_PROTECT_IO,                   //唤醒口选择
    .filter_enable = 1,                                    //滤波使能
};
#endif
...
const struct wakeup_param wk_param = {
    .port[1] = &key_port,
#if TCFG_CHARGE_BOX_ENABLE && TCFG_CHARGE_MOUDLE_OUTSIDE
    .port[2] = &usbin_det_port,
    .port[5] = &charge_full_det_port,
#endif
    .port[3] = &hall_det_port,
#if (TCFG_CHARGE_BOX_ENABLE && TCFG_WIRELESS_ENABLE)
    .port[4] = &wl_det_port,
#endif
#if (TCFG_TEST_BOX_ENABLE && (TCFG_CHARGESTORE_PORT != IO_PORTB_05))
    .port[6] = &uart_update_port,
#endif
#if (TCFG_CHARGE_BOX_ENABLE && TCFG_SHORT_PROTECT_ENABLE)
    .port[7] = &short_det_port,
#endif
    .sub = &sub_wkup,
    .charge = &charge_wkup,
...
#if TCFG_CHARGE_BOX_ENABLE && TCFG_CHARGE_MOUDLE_OUTSIDE
void usb_det_wakeup_set_edge(u8 edge)
{
    usbin_det_port.edge = edge;
    power_wakeup_port_set(2, &usbin_det_port);
}
#endif

#if TCFG_CHARGE_BOX_ENABLE && TCFG_WIRELESS_ENABLE
void wireless_port_set_wakeup_enable(u8 enable)
{
    if (enable) {
...
{
    hall_det_port.edge = edge;
    power_wakeup_port_set(3, &hall_det_port);
}
...
void short_det_wakeup_disable(void)
{
    power_wakeup_index_disable(7);
}

void short_det_wakeup_enable(void)
{
...
u8 get_power_on_status(void)
{
    ...
    power_io_list = (struct iokey_port *)iokey_data.port;
...
void chgbox_enter_soft_power_off(void)
{
    ...
    if (gpio_read(TCFG_USB_ONLE_DET_IO) && sys_info.status[USB_DET]==STATUS_ONLINE){
        usbin_det_port.edge = FALLING_EDGE;
    } else if (!gpio_read(TCFG_USB_ONLE_DET_IO) && sys_info.status[USB_DET]==STATUS_OFFLINE){
    ...
    if(sys_info.status[WIRELESS_DET]==STATUS_ONLINE) {
        ...
        power_wakeup_index_disable(4);
#if (TCFG_DCDC_CTRL_EN == 0)//无dcdcen,要关掉usb 唤醒，避免一直唤醒
#if TCFG_CHARGE_MOUDLE_OUTSIDE
        //外置充电USB唤醒
        power_wakeup_index_disable(2);
#else
        //内置充电USB唤醒
        LVCMP_EDGE_WKUP_EN(0);
#endif
#endif
    }
#endif
    ...
    //开机没电或者是过流保护中,只保留充电唤醒设备
    if ((sys_info.init_ok == 0) || (sys_info.current_limit == 1)) {
        power_wakeup_index_disable(1);
        power_wakeup_index_disable(3);
        log_info("softoff disable index 1 & 3\n");
    } else {
        power_wakeup_index_enable(1);
    }
#endif

#if TCFG_CHARGE_BOX_AUTO_WAKEUP
	if(!sys_info.lowpower_flag){
		set_softoff_wakeup_time_sec(AUTO_WAKEUP_TIME_SEC);
	}
#endif
...
void board_set_soft_poweroff(void)
{
...
#if TCFG_CHARGE_BOX_ENABLE && TCFG_CHARGE_BOX_SOFT_POWER_OFF_IO_CTRL_ENABLE
    //需要设置的IO
    gpio_set_pull_down(TCFG_BOOST_CTRL_IO, 1);//5v升压en要低,开下拉
    ...
    gpio_set_output_value(TCFG_HANDSHAKE_IO, 1); //握手需要拉高
#endif

#if (TCFG_WIRELESS_ENABLE && TCFG_DCDC_CTRL_EN)
    gpio_direction_output(TCFG_DCDC_EN_IO, 0); //dcdc en 需要关闭
#endif
...
static void port_wakeup_callback(u8 index, u32 gpio)
{
    /* log_info("wakeup: %d, %d\n", index, gpio); */
    switch (index) {
    case 1: break;//key
...
#if TCFG_WIRELESS_ENABLE
    case 4:
        wireless_io_wakeup_deal();
        break;
#endif
#if TCFG_SHORT_PROTECT_ENABLE
    case 7:
        chargebox_set_output_short();
        break;
#endif
#endif
#if (TCFG_TEST_BOX_ENABLE && (TCFG_CHARGESTORE_PORT != IO_PORTB_05))
...
static void charge_full_wakeup_callback(void)
{
    usb_charge_full_wakeup_deal();
}

static void ldoin_wakeup_callback(void)
{
    usb_wakeup_deal();
}
...
extern void port_edge_wkup_set_callback(void (*wakeup_callback)(u8 index, u32 gpio));
    port_edge_wkup_set_callback(port_wakeup_callback);

#if TCFG_CHARGE_BOX_ENABLE && (TCFG_CHARGE_MOUDLE_OUTSIDE == DISABLE)
extern void charge_set_callback(void (*wakup_callback)(void), void (*sub_callback)(void));
    charge_set_callback(charge_full_wakeup_callback, ldoin_wakeup_callback);
#endif
}
