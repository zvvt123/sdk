...
sec(.spi_code)
u8 spi_code_need_load_cache()
{
#if VM_SPI_CODE_SEC_AT_DATA_RAM
    return LOAD_SPI_CODE_NO_NEED;
#elif TCFG_VM_SPI_CODE_AT_RAM_DYANMIC
#if ((defined TCFG_FM_ENABLE) && TCFG_FM_ENABLE)
    extern u8 *get_vm_spi_code_run_addr();
    if (get_vm_spi_code_run_addr()) {
        return LOAD_SPI_CODE_NO_NEED;
    } else
#endif
    {
        return LOAD_SPI_REGION_CODE2CACHE;
    }
#else
    return LOAD_SPI_CODE2CACHE;
#endif
}

sec(.spi_code)
u8 is_spi_code_at_region()
{
