...
//                                  UI 配置                                        //
//*********************************************************************************//
#define TCFG_UI_ENABLE 						              0//DISABLE_THIS_MOUDLE 	//UI总开关
...
#if TRANS_MULTI_BLE_EN && SMART_BOX_EN
#error "please close usb one enalbe"
#endif
...
#if (AI_APP_PROTOCOL)
#define TCFG_BLUETOOTH_BACK_MODE			              0
#else
...
#if ((SMART_BOX_EN) && (TCFG_MIC_EFFECT_ENABLE) && (TCFG_AUDIO_OUT_EQ_ENABLE))
/*
 * 如果开了SMART_BOX_EN宏后，混响、高低音与DRC功能同时打开会导致在音乐模式运行时内存不够
 * TCFG_AUDIO_OUT_EQ_ENABLE关闭后，高低音和DRC功能都会被关闭
 * */
#error "高低音与混响功能不能同时使用"
#endif//((SMART_BOX_EN) && (TCFG_MIC_EFFECT_ENABLE) && (TCFG_AUDIO_OUT_EQ_ENABLE))
