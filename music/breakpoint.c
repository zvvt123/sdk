...
#if TCFG_FLASH_ENABLE
#endif

bool breakpoint_vm_read(struct __breakpoint *bp, char *logo) //从vm中读取断点信息
{
...
void breakpoint_vm_write(struct __breakpoint *bp, char *logo)//将断点信息写入vm
{
    if (bp == NULL || logo == NULL) {
        return ;
    }
    ...
    if (!strcmp(logo, "sd0")) {
        ...
        vm_id = CFG_USB_BREAKPOINT0;
    } else if (!strcmp(logo, "fat_nor")) {
        vm_id = CFG_FLASH_BREAKPOINT0;
    }else{
        return ;
    }
    printf("%s %s ok\n", logo, __FUNCTION__);
